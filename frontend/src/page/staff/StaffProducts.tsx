...
export default function StaffProducts() {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      // Capture Order ID for receipt
      if (orderRes.data?.id) {
        setCreatedOrderId(orderRes.data.id);
      } else if (orderRes.data?.code) {
        // Fallback/Check if backend returns full object structure differently
        // Based on controller it returns `result` which is the newOrder object
        setCreatedOrderId(orderRes.data.id);
      } else if (orderRes.data?.data?.id) {
        // Check if it's nested in data.data
      ...
      setCustomerEmail('');
      setSelectedMemberId('');
      setPaymentType('CASH'); // Reset payment type
  ...
  return (
    <div className="p-8">
      ...
      >
        ...
        />
        <div
          className={`absolute right-0 top-0 w-full max-w-md bg-white h-full shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${
        ...
        >
          <div className="p-6 border-b border-slate-200 flex items-center justify-between">
            ...
            <button
              onClick={() => setShowCart(false)}
              className="p-2 text-slate-400 hover:text-slate-600 rounded hover:bg-slate-100 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
            ...
            {items.length === 0 ? (
              ...
              <div className="space-y-4">
                {items.map((item) => (
                  <div
                    key={item.productId}
                    className="flex items-center gap-4 p-3 bg-slate-50 rounded animate-[fadeIn_0.3s_ease-out]"
                  >
